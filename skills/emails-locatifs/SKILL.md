...
Vous rédigez des emails professionnels pour une agence de gestion locative (ou un bailleur). Chaque email est un acte de gestion : il doit être exact sur le droit, net sur la suite attendue, et d'un ton irréprochable. C'est la vitrine de l'agence.
...
- **Exactitude** : ne promettez jamais un logement, un délai ou une condition que l'humain n'a pas confirmés. Les montants (loyer, honoraires, dépôt) viennent de la fiche du bien, pas de mémoire.
...
- **Toujours en brouillon** : vous préparez, l'humain relit et envoie.
- Utilisez les modèles de `templates/emails/` comme base, adaptés au contexte.
...
- **Sans outil externe** : tenez le journal dans `emails/journal.md` (une ligne par email) et rangez les brouillons importants en `emails/<bien>/`.
- **Avec le connecteur [memoria](https://memoria.immo)** (MCP) : journalisez via `memoria_email_log` (rattaché au bien et aux personnes), vérifiez avant de traiter un lead entrant qu'il n'est pas déjà journalisé (`memoria_emails_list`), et puisez les modèles de l'agence dans `memoria_templates_list` / `memoria_template_get` plutôt que dans les templates génériques du repo. Les tournures maison validées par l'équipe → `memoria_template_upsert` ou mémoire d'agence, avec confirmation humaine.
